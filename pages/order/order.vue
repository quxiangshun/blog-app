			...
			<view class="option center space-between">
				...
				<view>
					<!-- 
					 ...
					 教师关闭：显示删除按钮
					 -->
					<button v-if="item.status === 1" size="mini">取消订单</button>
					<button v-if="item.status === 1" type="warn" size="mini">立即支付</button>
					<button v-if="item.status === 3" size="mini">删除订单</button>
				</view>
			</view>
		</view>
